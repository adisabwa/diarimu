      ...
      <el-select size="small" v-model="dates" placeholder="Pilih Tanggal"
        ...
        <template #footer>
          <el-button text bg size="small"
            @click="dateFunction(addDay(dataOptions[dataOptions.length - 1]?.start,'-1'))">
            Tanggal Sebelumnya
          </el-button>
        </template>
      </el-select>
...
export default {
  ...
  watch:{
    async tipe(val){
      ...
      this.dates = this.getValue(this.dataOptions[0]);
      this.selectKey++
      this.getChart()
    }
  },
    ...
    async dateFunction(last, number = 1){
      ...
      console.log(newData)
      let newOpt = [...this.dataOptions, ...newData]
      this.dataOptions = JSON.parse(JSON.stringify(newOpt))
