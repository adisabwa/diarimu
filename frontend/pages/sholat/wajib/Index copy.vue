  ...
  methods: {
    ...
    handleAfterScroll(){
      ...
      if (vm.editTanggal == true)
        return
      let body = this.jquery('#body-scroll')[0]
      let right = this.jquery('#body2')[0]
      console.log(body.scrollLeft, right?.offsetLeft)
      if ((Math.floor(body.scrollLeft / 10)) == 0) {
        this.changeTanggalData(-1)
        this.setHeaderToCenter()
      } else if (Math.floor(body?.scrollLeft / 10) == Math.floor(right?.offsetLeft / 10)) {
        this.changeTanggalData(1)
        this.setHeaderToCenter()
      }
      // setTimeout(() => {
