    ...
    public function upload()
    {
        ...
        while (!empty($no)) {
            ...
            if ($this->validation->run($data)) {
                $check = $this->iqabModel->where($data)->first();

                if (!empty($check)) {
                    $data["created_by"] =  userdata()->id;
                    $save = $this->iqabModel->update($check['id'], $data);
                    $data['id'] = $check['id'];

                } else {
                    $save = $this->iqabModel->insert($data);
                    $data['id'] = $this->iqabModel->insertID();
                }
