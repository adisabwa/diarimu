...
class PelanggaranIqab extends BaseData
{
    ...
    public function __construct()
    {
        ...
        $this->model = model('PelanggaranIqabModel');
    }
    ...
    public function template()
    {
        ...
        $spreadsheet->getActiveSheet()->setCellValue('B1', 'Label PelanggaranIqab');
    ...
    public function upload()
    {
        ...
        while (!empty($no)) {
            ...
            if ($this->validation->run($data)) {
                $check = $this->pelanggaranIqabModel->where($data)->first();

                if (!empty($check)) {
                    $data["created_by"] =  userdata()->id;
                    $save = $this->pelanggaranIqabModel->update($check['id'], $data);
                    $data['id'] = $check['id'];

                } else {
                    $save = $this->pelanggaranIqabModel->insert($data);
                    $data['id'] = $this->pelanggaranIqabModel->insertID();
                }
